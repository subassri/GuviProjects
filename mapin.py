...
create_query = """ 
CREATE TABLE IF NOT EXISTS mapinsunew (
    State VARCHAR(100),
...
# Insert data
path = r'C:\Users\Administrator\Desktop\Miniproj\venv\pulse\data\map\insurance\hover\country\india'
data_list = []
...
for index, row in df.iterrows():
    insert_query = """ 
    INSERT INTO mapinsunew (State, District, Count, Amount)
    VALUES (%s, %s, %s, %s)
    """
...
# Count rows
count_query = "SELECT COUNT(*) FROM mapinsunew"
cursor.execute(count_query)
count = cursor.fetchone()
print(f"Number of rows in mapinsunew table: {count[0]}")
